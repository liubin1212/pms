...
<script type="text/javascript">
	//发送异步请求查询通知公告的最新三条数据
	$.ajax({
		url:"<%=basePath%>notice/findNewThreeNotice",
		type:"get",
		success:function (msg) {
			if(msg.map.statusCode == 200){
				$(msg.map.list).each(function (index,item) {
					var li = $("<li class=\"ue-clear\">\n" +
                        "<span>"+moment(item.ndate).format("YYYY-MM-DD")+"</span>&nbsp;&nbsp;&nbsp;" +
						"<a onclick='findNotice("+item.nid+")' href=\"#\"class=\"notice-title\">"+item.ntitle+"</a>\n" +
                        "</li><p>");
					$("#addLi").append(li);
                })
			}
	...
	function findNotice(nid) {
		$.ajax({
			url:"<%=basePath%>notice/findNotice/"+nid,
			type:"get",
			success:function (msg) {
