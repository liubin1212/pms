...
<tr>
 <td height="26" background="skin/images/newlinebg3.gif">
  ...
  <td>
    <input type='button' class="coolbg np" onClick="location='<%=basePath%>project-function-add.jsp';" value='添加新功能' />
 </td>
 </tr>
</table>
</td>
...
<c:forEach items="${list}" var="function" varStatus="index">
    <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
        ...
        <td align="center"><a href=''><u>${function.module.modname}</u></a></td>
        <td align="center"><a href=''><u>${function.analysis.title}</u></a></td>
        <td align="center"><a href=''><u>${function.analysis.proname}</u></a></td>
        <td>
            <c:if test="${function.level == '高'}">高</c:if>
            <c:if test="${function.level == '中'}">中</c:if>
            <c:if test="${function.level == '低'}">低</c:if>
        </td>
        <td>
            <fmt:formatDate value="${function.analysis.addtime}" pattern="yyyy-MM-dd"></fmt:formatDate>
        </td>
        <td>
            <fmt:formatDate value="${function.analysis.updatetime}" pattern="yyyy-MM-dd"></fmt:formatDate>
        </td>
