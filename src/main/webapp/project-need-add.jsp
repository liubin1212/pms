...
<form id="form2" action="<%=basePath%>ana/addAnalysis" method="post">
...
<tr >
	...
	<td  align='left' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
		<select name="proname" id="project" onchange="changeId(this.value)">
			<option>请选择...</option>
		</select>
	</td>
</tr>
    ...
    $.ajax({
		...
		success:function (msg) {
			$(msg).each(function (index,item) {
				var pro = $("<option value='"+item.pname+"'>"+item.pname+"</option>");
				$("#project").append(pro);
            })
        }
	});
	function changeId(pname) {
        $.ajax({
            url:"<%=basePath%>pro/findJsonProjectByPname?pname="+pname,
            type:"get",
            dataType:"json",
            success:function (msg) {
				alert(msg.pid);
